      ...
      <div className="w-full max-w-[680px] bg-white h-full overflow-y-auto shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-7">
          ...
          <div className="grid grid-cols-2 gap-3">
            {([['Date', fmtDate(ev.date)],['Time', ev.time||'—'],['Duration', ev.duration||'—'],['Venue', ev.venue_name||'—'],['Address', ev.venue_address||'—'],['Artist', ev.artist_name||'—'],['Tickets For', ev.tickets_needed_for||'—'],['Created', fmtDate(ev.createdAt||ev.created_at)]] as [string,string][]).map(([l,v]) => (
          ...
          {ev.guide && <Section icon={<Users size={14}/>} title="Event Guide"><div className="grid grid-cols-2 gap-3">{ev.guide.languages?.length ? <GF label="Languages" value={ev.guide.languages.join(', ')}/> : null}{ev.guide.min_age != null && <GF label="Min Age" value={String(ev.guide.min_age)}/>}{ev.guide.venue_type && <GF label="Venue Type" value={ev.guide.venue_type}/>}<GF label="Kid Friendly" value={ev.guide.is_kid_friendly?'Yes':'No'}/><GF label="Pet Friendly" value={ev.guide.is_pet_friendly?'Yes':'No'}/>{ev.guide.facilities?.length ? <div className="col-span-2"><p className="text-[10px] uppercase font-semibold text-zinc-400 mb-1">Facilities</p><div className="flex flex-wrap gap-1.5">{ev.guide.facilities.map(f => <span key={f} className="text-xs bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-full">{f}</span>)}</div></div> : null}</div></Section>}
          {ev.artist_image_url && <Section icon={<Tag size={14}/>} title="Artist"><div className="flex items-center gap-4"><img src={ev.artist_image_url} alt={ev.artist_name} className="w-16 h-16 rounded-[10px] object-cover border border-zinc-100"/>{ev.artist_name && <span className="text-sm font-semibold text-black">{ev.artist_name}</span>}</div></Section>}
