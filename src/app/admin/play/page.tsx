...
function AdminPlayDetailView({ ev, onStatus, onUpdate, onDelete, onBack }: {
    ...
    const opMatch = (ev.opening_time || '').match(/(\d+):(\d+)\s+(AM|PM)/i);
    const clMatch = (ev.closing_time || '').match(/(\d+):(\d+)\s+(AM|PM)/i);
    
    const [timeHour, setTimeHour] = useState(opMatch ? opMatch[1] : '');
    const [timeMinute, setTimeMinute] = useState(opMatch ? opMatch[2] : '');
    const [timePeriod, setTimePeriod] = useState(opMatch ? opMatch[3].toUpperCase() : '');
    const [closeHour, setCloseHour] = useState(clMatch ? clMatch[1] : '');
    const [closeMinute, setCloseMinute] = useState(clMatch ? clMatch[2] : '');
    const [closePeriod, setClosePeriod] = useState(clMatch ? clMatch[3].toUpperCase() : '');
    ...
    const [facilities, setFacilities] = useState<string[]>(ev.guide?.facilities || []);
    const [petFriendly, setPetFriendly] = useState(ev.guide?.is_pet_friendly ? 'Yes' : 'No');
    const [maxDuration, setMaxDuration] = useState(ev.guide?.max_duration || '');
    const [outsideFood, setOutsideFood] = useState(ev.guide?.outside_food_allowed ? 'Yes' : 'No');
    const [venueLocationType, setVenueLocationType] = useState(ev.guide?.venue_location_type || '');
    const [surfaceType, setSurfaceType] = useState(ev.guide?.surface_type || '');
    const [cancellations, setCancellations] = useState(ev.guide?.cancellations_allowed ? 'Yes' : 'No');
    const [changingRooms, setChangingRooms] = useState(ev.guide?.changing_rooms_available ? 'Yes' : 'No');
    const [equipmentRentals, setEquipmentRentals] = useState(ev.guide?.equipment_rentals_available ? 'Yes' : 'No');
    ...
    }, [ev, venueName, venueAddress, instagramLink, googleMapLink, openingTime, timeHour, timeMinute, timePeriod, closeHour, closeMinute, closePeriod, 
    ...
    const handleReplaceGalleryImage = async (file: File, index: number) => {
        ...
        try {
            const url = await uploadMedia(file);
            setGalleryUrls(prev => prev.map((img, i) => (i === index ? url : img)));
        } catch {
            toast.error('Gallery image replace failed. Try again.');
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setHasChanges(false);
            setTimeout(() => router.push(`/admin/play/${id}/format?mode=view`), 1800);
        } catch (err) {
                ...
                <div className="flex items-center gap-4">
                    ...
                    {isEditMode && hasChanges && (
                        <button
                            ...
                            className="px-6 py-2 rounded-lg bg-[#E7C200] text-black font-bold text-sm hover:bg-[#d6b300] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                    ...
                    >
                        {isEditMode ? <><Check size={16} /> View Mode</> : <><Edit3 size={16} /> Edit Details</>}
                    </button>
                    {!isEditMode && (
                        <button
                            onClick={() => router.push(`/admin/play/${id}/format?mode=view`)}
                            className="px-6 py-2 rounded-lg bg-black text-white font-bold text-sm hover:bg-zinc-800"
                        >
                            Next: Pricing Format
                        </button>
                    )}
                ...
                <div className="max-w-[1920px] mx-auto px-10 pt-10">
                    ...
                    <div className="mb-6">
                        <h1 className="text-[40px] font-medium leading-[44px] text-black" style={{ fontFamily: 'var(--font-anek-latin)' }}>
                            {isEditMode ? 'Edit play venue' : 'View play venue'}
                        </h1>
                        <p className="text-[25px] font-medium text-[#686868] mt-2">
                            {isEditMode ? 'Make changes and save to update this venue.' : 'Overview of venue details on Ticpin.'}
                        </p>
                    </div>
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-[20px] font-medium text-[#686868]">Opening time <span className="text-[#E7C200]">*</span></span>
                                        <div className="flex items-center gap-3 w-full ml-4">
                                            <div className="relative border border-[#686868] rounded-[10px] h-[64px] flex-1 flex items-center px-4">
                                                <select value={timeHour} onChange={e => setTimeHour(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[20px] text-black ${!isEditMode ? 'cursor-default' : ''}`}>
                                            ...
                                            <div className="relative border border-[#686868] rounded-[10px] h-[64px] flex-1 flex items-center px-4">
                                                <select value={timeMinute} onChange={e => setTimeMinute(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[20px] text-black ${!isEditMode ? 'cursor-default' : ''}`}>
                                            ...
                                            <div className="flex border border-[#686868] rounded-[10px] h-[64px] overflow-hidden">
                                                {['AM', 'PM'].map(period => (
                                                    <button
                                                        ...
                                                        disabled={!isEditMode}
                                                        className={`w-[60px] text-[18px] font-semibold transition-colors ${timePeriod === period ? 'bg-[#FFFCED] text-black' : 'bg-transparent text-[#686868]'} ${isEditMode ? 'hover:bg-zinc-100' : 'cursor-default'}`}
                                                    >
                                                        {period}
                                                    </button>
                                                ))}
                                    ...
                                    {/* Closing Time */}
                                    <div className="flex items-center justify-between">
                                        <span className="text-[20px] font-medium text-[#686868]">Closing time <span className="text-[#E7C200]">*</span></span>
                                        <div className="flex items-center gap-3 w-full ml-4">
                                            <div className="relative border border-[#686868] rounded-[10px] h-[64px] flex-1 flex items-center px-4">
                                                <select value={closeHour} onChange={e => setCloseHour(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[20px] text-black ${!isEditMode ? 'cursor-default' : ''}`}>
                                                    <option value="">HH</option>
                                                    {Array.from({ length: 12 }, (_, i) => String(i + 1).padStart(2, '0')).map(h => (
                                                        <option key={h} value={h}>{h}</option>
                                                    ))}
                                            ...
                                            <span className="text-[28px] font-semibold text-[#686868]">:</span>
                                            <div className="relative border border-[#686868] rounded-[10px] h-[64px] flex-1 flex items-center px-4">
                                                <select value={closeMinute} onChange={e => setCloseMinute(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[20px] text-black ${!isEditMode ? 'cursor-default' : ''}`}>
                                                    <option value="">MM</option>
                                                    {Array.from({ length: 60 }, (_, i) => String(i).padStart(2, '0')).map(m => (
                                                        <option key={m} value={m}>{m}</option>
                                                    ))}
                                            ...
                                            <div className="flex border border-[#686868] rounded-[10px] h-[64px] overflow-hidden">
                                                {['AM', 'PM'].map(period => (
                                                    <button
                                                        ...
                                                        disabled={!isEditMode}
                                                        className={`w-[60px] text-[18px] font-semibold transition-colors ${closePeriod === period ? 'bg-[#FFFCED] text-black' : 'bg-transparent text-[#686868]'} ${isEditMode ? 'hover:bg-zinc-100' : 'cursor-default'}`}
                                                    >
                                                        {period}
                                                    </button>
                                                ))}
                            ...
                            <div className="space-y-6">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="relative w-[840px]">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => isEditMode && setShowFacilitiesDropdown(d => !d)}
                                            className={`w-full border border-[#686868] rounded-[10px] min-h-[64px] flex items-center flex-wrap gap-2 px-4 py-2 text-left ${!isEditMode ? 'cursor-default opacity-70' : ''}`}
                                        >
                                            {facilities.length === 0 ? (
                                        ...
                                        {/* Dropdown */}
                                        {isEditMode && showFacilitiesDropdown && (
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={maxDuration} onChange={e => setMaxDuration(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={outsideFood} onChange={e => setOutsideFood(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                            <option value="">Select option</option>
                                            <option value="Yes">Yes</option>
                                            <option value="No">No</option>
                                        </select>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={venueLocationType} onChange={e => setVenueLocationType(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                            <option value="">Select option</option>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={surfaceType} onChange={e => setSurfaceType(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={cancellations} onChange={e => setCancellations(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                            <option value="">Select option</option>
                                            <option value="Yes">Yes</option>
                                            <option value="No">No</option>
                                        </select>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={changingRooms} onChange={e => setChangingRooms(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                            <option value="">Select option</option>
                                            <option value="Yes">Yes</option>
                                            <option value="No">No</option>
                                        </select>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="relative border border-[#686868] rounded-[10px] h-[64px] w-[840px] flex items-center px-6">
                                        <select value={equipmentRentals} onChange={e => setEquipmentRentals(e.target.value)} disabled={!isEditMode} className={`w-full appearance-none bg-transparent outline-none text-[25px] ${!isEditMode ? 'cursor-default' : ''}`}>
                                            <option value="">Select option</option>
                                            <option value="Yes">Yes</option>
                                            <option value="No">No</option>
                                        </select>
                                        ...
                                        <div className="flex flex-wrap gap-3 mb-4">
                                            {galleryUrls.map((u, i) => (
                                                <div key={i} className="relative w-[80px] h-[80px]">
                                                    ...
                                                    {isEditMode && (
                                                        <div className="absolute inset-0 bg-black/45 opacity-0 hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1 rounded-[8px]">
                                                            <label
                                                                htmlFor="upload-gallery-replace"
                                                                onClick={() => setReplacingGalleryIndex(i)}
                                                                className="text-[11px] px-2 py-0.5 bg-white/90 rounded cursor-pointer"
                                                            >
                                                                {uploading.gallery_replace && replacingGalleryIndex === i ? 'Uploading...' : 'Replace'}
                                                            </label>
                                                            <button
                                                                onClick={() => setGalleryUrls(prev => prev.filter((_, idx) => idx !== i))}
                                                                className="text-[11px] px-2 py-0.5 bg-red-500 text-white rounded"
                                                            >
                                                                Remove
                                                            </button>
                                                        </div>
                    ...
                    <div className="flex flex-col items-center mt-8 mb-20 gap-4">
                        ...
                        {!isEditMode && (
                            <button 
                                onClick={() => router.push(`/admin/play/${id}/format?mode=view`)}
                                className="bg-black text-white rounded-[15px] w-full py-4 text-[25px] font-medium hover:bg-zinc-800 transition-colors"
                            >
                                Next: Pricing Format
                            </button>
                        )}
                        {isEditMode && hasChanges && (
                            <button 
                                onClick={handleSave} 
                                disabled={submitLoading} 
                                className="bg-black text-white rounded-[15px] w-full py-4 text-[25px] font-medium disabled:opacity-50 hover:bg-zinc-800 transition-colors"
                            >
