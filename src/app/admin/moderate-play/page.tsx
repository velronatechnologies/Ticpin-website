...
import { Check, X, MapPin, Tag, ExternalLink, LayoutGrid, Edit2, Trash2 } from 'lucide-react';
...
interface PlayVenue {
    ...
    play_options: { sport: string; court_type: string; surface: string; price_per_slot: number }[];
}

export default function ModeratePlayPage() {
    const { token, isAdmin, isLoggedIn } = useStore();
    const router = useRouter();
    const [venues, setVenues] = useState<PlayVenue[]>([]);
    ...
    const fetchVenues = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/play?limit=50&all=true&status=pending`, {
                headers: {
    ...
    const handleAction = async (id: string, status: string, reason?: string) => {
        ...
        if (!confirm(`Are you sure you want to ${status} this play venue?`)) return;

        try {
            const params = new URLSearchParams({ status });
            if (reason) params.append('reason', reason);
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/admin/play/${id}/approve?${params.toString()}`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Are you sure you want to PERMANENTLY delete this play venue? This action cannot be undone.')) return;

        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/admin/play/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            if (response.ok) {
                ...
                console.error('Error deleting play venue');
            }
        } catch (error) {
    ...
    if (loading) return <div className="p-20 text-center">Loading pending play venues...</div>;
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="flex justify-between items-center">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-bold text-zinc-900">Play Venue Moderation</h1>
                        <p className="text-zinc-500 font-medium">Review and approve pending play venue submissions</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6">
                    {venues.length > 0 ? (
                            ...
                            <div key={venue.id} className="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row">
                                ...
                                <div className="flex-1 p-6 space-y-4">
                                    ...
                                    <div className="flex items-center gap-6 py-4 border-y border-zinc-100">
                                        <div className="space-y-1">
                                            <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Sports Offered</p>
                                            <div className="flex flex-wrap gap-2">
                                                {venue.play_options?.map((opt, i) => (
                                                    <span key={i} className="px-2 py-1 bg-zinc-100 rounded-lg text-xs font-medium text-zinc-700">
                                                        {opt.sport} ({opt.court_type}) - ₹{opt.price_per_slot}
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex flex-wrap justify-between items-center pt-2 gap-4">
                                        <div className="flex items-center gap-4">
                                            <a href={`/play/${venue.id}`} target="_blank" className="text-emerald-600 text-sm font-bold flex items-center gap-1 hover:underline">
                                                <ExternalLink size={14} /> Preview Page
                                            </a>
                                            <a href={`/admin/edit-play/${venue.id}`} className="text-zinc-600 text-sm font-bold flex items-center gap-1 hover:underline">
                                                <Edit2 size={14} /> Edit
                                            </a>
                                            <button
                        ...
                        <div className="p-20 text-center bg-white rounded-3xl border border-zinc-200">
                            <p className="text-zinc-500 text-lg">No pending play venues found.</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full space-y-6 shadow-2xl">
                        <div className="space-y-2">
                            ...
                            <p className="text-zinc-500">Please provide a reason why this play venue is being rejected. This will be visible to the organizer.</p>
                        </div>
                        <textarea
