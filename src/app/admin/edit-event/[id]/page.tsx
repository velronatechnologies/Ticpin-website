...
function AdminEditEventForm() {
    ...
    const fetchEvent = async () => {
        ...
        if (response.success && response.data) {
            ...
            if (data.start_datetime) data.start_datetime = new Date(data.start_datetime).toISOString().slice(0, 16);
            if (data.end_datetime) data.end_datetime = new Date(data.end_datetime).toISOString().slice(0, 16);
            ...
            addToast('Failed to fetch event details', 'error');
            router.push('/admin/edit-event');
        }
        setLoading(false);
    };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: string, index?: number) => {
        ...
        try {
            ...
            if (data.status === 200 && data.data.url) {
                if (target === 'hero') setFormData((prev: any) => ({ ...prev, images: { ...prev.images, hero: data.data.url } }));
                else if (target === 'poster') setFormData((prev: any) => ({ ...prev, images: { ...prev.images, poster: data.data.url } }));
                    ...
                    setFormData((prev: any) => ({ ...prev, artists: newArtists }));
                }
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            const response = await eventsApi.adminUpdate(id as string, {
                ...formData,
            ...
            if (response.success) {
                ...
                addToast(response.message || 'Error updating event', 'error');
            }
    ...
    const removeArtist = (index: number) => setFormData({ ...formData, artists: formData.artists.filter((_: any, i: number) => i !== index) });
    ...
    const removeTicket = (index: number) => setFormData({ ...formData, tickets: formData.tickets.filter((_: any, i: number) => i !== index) });
        ...
        <div className="min-h-screen bg-[#FBFBFF] font-[family-name:var(--font-anek-latin)]">
            ...
            <header className="sticky top-0 z-50 bg-white border-b border-zinc-200 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.push('/admin/edit-event')} className="p-2 hover:bg-zinc-100 rounded-full transition-colors">
                        <ChevronLeft size={24} />
                    </button>
                    <div>
                        <h1 className="text-xl font-bold text-zinc-900">Edit Event</h1>
                        <p className="text-sm text-purple-600 font-medium">{formData.title || formData.name}</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <button onClick={() => router.push('/admin/edit-event')} className="px-6 py-2.5 text-zinc-600 font-bold hover:bg-zinc-100 rounded-xl transition-all">Cancel</button>
                    <button onClick={handleSubmit} disabled={saving} className="bg-purple-600 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-purple-700 hover:shadow-lg disabled:opacity-50 transition-all flex items-center gap-2">
                ...
                <form onSubmit={handleSubmit} className="space-y-12">
                    ...
                    <Section icon={<FileText size={20} />} title="Basic Information">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <InputField label="Event Title" value={formData.title || formData.name} onChange={v => setFormData({ ...formData, title: v })} placeholder="eg. AR Rahman Live Concert" required />
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    className="w-full h-14 px-5 bg-white border border-zinc-200 rounded-2xl text-[16px] font-medium focus:outline-none focus:border-purple-600 focus:ring-4 focus:ring-purple-600/5 transition-all outline-none"
                                >
                        ...
                        <div className="mt-8 space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full h-40 p-5 bg-white border border-zinc-200 rounded-2xl text-[16px] font-medium focus:outline-none focus:border-purple-600 focus:ring-4 focus:ring-purple-600/5 transition-all outline-none resize-none"
                    ...
                    <Section icon={<Ticket size={20} />} title="Ticket Inventory">
                        <div className="space-y-6">
                            {formData.tickets?.map((ticket: any, i: number) => (
                            ...
                            <button type="button" onClick={addTicket} className="w-full py-6 border-2 border-dashed border-zinc-200 rounded-3xl text-zinc-400 font-bold hover:border-purple-600 hover:text-purple-600 transition-all flex items-center justify-center gap-2">
                    ...
                    <Section icon={<Users size={20} />} title="Performing Artists">
                        <div className="space-y-6">
                            {formData.artists?.map((artist: any, i: number) => {
                                const isExpanded = expandedArtists.includes(i);
                                return (
                                    <div key={i} className="bg-zinc-50 border border-zinc-200 p-6 rounded-3xl relative group">
                                        <button type="button" onClick={() => removeArtist(i)} className="absolute -top-3 -right-3 w-8 h-8 bg-white border border-zinc-200 text-red-500 rounded-full flex items-center justify-center hover:bg-red-50 transition-colors shadow-sm z-10">
                                            <Trash2 size={16} />
                                        </button>
                                        <div className="flex flex-col md:flex-row gap-8">
                                            ...
                                            <div className="flex-1 space-y-4">
                                                <div className="flex justify-between items-center">
                                                    <h4 className="font-bold text-zinc-900">Artist #{i + 1}</h4>
                                                    <button
                                                        type="button"
                                                        onClick={() => toggleArtistExpansion(i)}
                                                        className="text-sm text-purple-600 font-bold hover:underline"
                                                    >
                                                        {isExpanded ? 'Show Less' : 'Edit Professional Details'}
                                                    </button>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    <InputField label="Genre" value={artist.genre} onChange={v => {
                                                        const a = [...formData.artists]; a[i].genre = v; setFormData({ ...formData, artists: a });
                                                    }} placeholder="eg. Rock, Jazz" />
                                                    <InputField label="Location" value={artist.location} onChange={v => {
                                                        const a = [...formData.artists]; a[i].location = v; setFormData({ ...formData, artists: a });
                                                    }} placeholder="eg. Mumbai" />
                                                </div>

                                                {isExpanded && (
                                                    <div className="space-y-6 pt-6 border-t border-zinc-200 animate-in fade-in slide-in-from-top-2">
                                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                            <InputField label="Followers" type="number" value={artist.follower_count} onChange={v => { const a = [...formData.artists]; a[i].follower_count = Number(v); setFormData({ ...formData, artists: a }); }} />
                                                            <InputField label="Rating" type="number" value={artist.rating} onChange={v => { const a = [...formData.artists]; a[i].rating = Number(v); setFormData({ ...formData, artists: a }); }} />
                                                            <InputField label="Experience (Y)" type="number" value={artist.experience_years} onChange={v => { const a = [...formData.artists]; a[i].experience_years = Number(v); setFormData({ ...formData, artists: a }); }} />
                                                        </div>

                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                            <InputField label="Booking Email" value={artist.contact_email} onChange={v => { const a = [...formData.artists]; a[i].contact_email = v; setFormData({ ...formData, artists: a }); }} />
                                                            <InputField label="Booking Phone" value={artist.contact_phone} onChange={v => { const a = [...formData.artists]; a[i].contact_phone = v; setFormData({ ...formData, artists: a }); }} />
                                                        </div>
                                                        ...
                                                        <InputField
                                                            ...
                                                            onChange={v => {
                                                                ...
                                                                a[i].specialties = v.split(',').map(s => s.trim());
                                                                setFormData({ ...formData, artists: a });
                                                            }}
                                                            placeholder="eg. Live Band, DJ, Wedding Sets"
                                                        />

                                                        <div className="space-y-2">
                                                            <label className="text-[12px] font-bold text-[#686868] uppercase tracking-wider">Social Links (newline separated)</label>
                                                            <textarea
                                                                value={artist.social_links?.join('\n')}
                                                                onChange={e => {
                                                                    const a = [...formData.artists];
                                                                    a[i].social_links = e.target.value.split('\n');
                                                                    setFormData({ ...formData, artists: a });
                                                                }}
                                                        ...
                                                        <div className="flex items-center gap-6 p-4 bg-white border border-zinc-200 rounded-2xl">
                                                            <label className="flex items-center gap-2 cursor-pointer">
                                                                <input type="checkbox" checked={artist.is_verified} onChange={e => { const a = [...formData.artists]; a[i].is_verified = e.target.checked; setFormData({ ...formData, artists: a }); }} className="w-5 h-5 rounded text-purple-600" />
                                                                <span className="text-sm font-bold text-zinc-700">Verified Artist Badge</span>
                                                            </label>
                                                        </div>
                                                    </div>
                                                )}
                            ...
                            <button type="button" onClick={addArtist} className="w-full py-4 border-2 border-dashed border-zinc-200 rounded-[28px] text-zinc-400 font-bold hover:border-purple-600 hover:text-purple-600 transition-all flex items-center justify-center gap-2">
                    ...
                    <Section icon={<HelpCircle size={20} />} title="FAQs">
                        <div className="space-y-4">
                            ...
                            <button type="button" onClick={addFaq} className="text-purple-600 font-bold flex items-center gap-2 text-sm"><Plus size={16} /> Add FAQ</button>
                        </div>
                    </Section>
                    ...
                    <Section icon={<Info size={20} />} title="Terms & Conditions">
                        <div className="space-y-4">
                            ...
                            <button type="button" onClick={addTerm} className="text-purple-600 font-bold flex items-center gap-2 text-sm"><Plus size={16} /> Add Term</button>
                        </div>
                    </Section>
                </form>
            </div>
    ...
    return (
        <section className="bg-white border border-zinc-200 shadow-sm rounded-[32px] overflow-hidden">
            <div className="px-8 py-6 border-b border-zinc-100 flex items-center gap-3 bg-zinc-50/50">
                <div className="w-10 h-10 bg-white border border-zinc-200 rounded-xl flex items-center justify-center text-purple-600 shadow-sm">{icon}</div>
    ...
    return (
        <div className="space-y-2">
            ...
            <input
                ...
                className="w-full h-14 px-5 bg-white border border-zinc-200 rounded-2xl text-[16px] font-medium focus:outline-none focus:border-purple-600 focus:ring-4 focus:ring-purple-600/5 transition-all outline-none placeholder:text-zinc-300"
