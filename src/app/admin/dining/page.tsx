...
function LinkChip({ href, label }: { href: string; label: string }) {
  return <a href={href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-[#E7C200] border border-yellow-200 bg-yellow-50 px-2.5 py-1 rounded-full hover:bg-yellow-100 transition-colors font-medium"><ExternalLink size={10}/>{label}</a>;
}
...
function PreviewModal({ ev, onClose, onStatusChange }: { ev: AdminListing; onClose: () => void; onStatusChange: (id: string, s: ListingStatus) => void }) {
  ...
  const handleStatus = async (status: ListingStatus) => {
    ...
    try { await adminApi.updateDiningStatus(id, status); onStatusChange(id, status); } catch (e) { alert(e instanceof Error ? e.message : 'Failed'); } finally { setUpdating(null); }
  };
  return (
      ...
      <div className="w-full max-w-[680px] bg-white h-full overflow-y-auto shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-7">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-black">{ev.name || 'Untitled Restaurant'}</h2>
            <div className="flex flex-wrap gap-2">
              {ev.category && <span className="text-xs bg-yellow-50 text-yellow-700 border border-yellow-100 px-2.5 py-1 rounded-full font-medium">{ev.category}{ev.sub_category ? ` · ${ev.sub_category}` : ''}</span>}
          ...
          <div className="grid grid-cols-2 gap-3">
            {([['Date', fmtDate(ev.date)],['Time', ev.time||'—'],['Duration', ev.duration||'—'],['Venue', ev.venue_name||'—'],['Address', ev.venue_address||'—'],['Created', fmtDate(ev.createdAt||ev.created_at)]] as [string,string][]).map(([l,v]) => (
          ...
          {(ev.google_map_link || ev.instagram_link || ev.google_business_link) && <Section icon={<ExternalLink size={14}/>} title="Links"><div className="flex flex-wrap gap-2">{ev.google_map_link && <LinkChip href={ev.google_map_link} label="Google Maps"/>}{ev.instagram_link && <LinkChip href={ev.instagram_link} label="Instagram"/>}{ev.google_business_link && <LinkChip href={ev.google_business_link} label="Google Business"/>}</div></Section>}
          {ev.description && <Section icon={<BookOpen size={14}/>} title="Description"><p className="text-sm text-zinc-600 leading-relaxed whitespace-pre-wrap">{ev.description}</p></Section>}
          {ev.event_instructions && <Section icon={<Info size={14}/>} title="Instructions"><p className="text-sm text-zinc-600 leading-relaxed whitespace-pre-wrap">{ev.event_instructions}</p></Section>}
          {gallery.length > 0 && <Section icon={<Tag size={14}/>} title={`Gallery (${gallery.length})`}><div className="grid grid-cols-3 gap-2">{gallery.map((url,i) => <div key={i} className="aspect-square rounded-[10px] overflow-hidden bg-zinc-100"><img src={url} alt={`g${i}`} className="w-full h-full object-cover"/></div>)}</div></Section>}
          {ev.menu_urls?.length ? <Section icon={<BookOpen size={14}/>} title={`Menu (${ev.menu_urls.length})`}><div className="grid grid-cols-3 gap-2">{ev.menu_urls.map((url,i) => <div key={i} className="aspect-square rounded-[10px] overflow-hidden bg-zinc-100"><img src={url} alt={`menu${i}`} className="w-full h-full object-cover"/></div>)}</div></Section> : null}
          ...
          {ev.points_of_contact?.length ? <Section icon={<Phone size={14}/>} title="Points of Contact"><div className="space-y-2">{ev.points_of_contact.map((c,i) => <div key={i} className="flex items-center gap-3 bg-zinc-50 rounded-[10px] p-3"><div className="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center flex-shrink-0"><Users size={14} className="text-yellow-600"/></div><div><p className="text-sm font-semibold text-black">{c.name}</p><div className="flex gap-3 mt-0.5">{c.email && <span className="flex items-center gap-1 text-xs text-zinc-500"><Mail size={10}/>{c.email}</span>}{c.mobile && <span className="flex items-center gap-1 text-xs text-zinc-500"><Phone size={10}/>{c.mobile}</span>}</div></div></div>)}</div></Section> : null}
          ...
          {ev.prohibited_items?.length ? <Section icon={<AlertTriangle size={14}/>} title="Prohibited Items"><ul className="space-y-1">{ev.prohibited_items.map((item,i) => <li key={i} className="flex items-center gap-2 text-sm text-zinc-600"><span className="w-1.5 h-1.5 rounded-full bg-red-400"/>{item}</li>)}</ul></Section> : null}
          {ev.terms && <Section icon={<BookOpen size={14}/>} title="Terms"><p className="text-sm text-zinc-600 leading-relaxed whitespace-pre-wrap">{ev.terms}</p></Section>}
...
export default function AdminDiningPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { const data = await adminApi.listDining(); setListings(Array.isArray(data) ? data : []); }
  ...
  const handleStatus = async (id: string, status: ListingStatus) => {
    setUpdating(id);
    try { await adminApi.updateDiningStatus(id, status); setListings(prev => prev.map(i => getId(i) === id ? { ...i, status } : i)); setPreview(prev => prev && getId(prev) === id ? { ...prev, status } : prev); }
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await adminApi.deleteDining(id); setListings(prev => prev.filter(i => getId(i) !== id)); }
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #ECE8FD 0%, #FFFFFF 100%)', zoom: 0.85 }}>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-[20px] shadow-2xl p-8 w-full max-w-sm space-y-4">
            ...
            <h3 className="text-lg font-bold text-black text-center">Delete this dining listing?</h3>
      ...
      <div className="max-w-[1240px] mx-auto px-8 py-10">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-[26px] font-bold text-black">Dining Management</h1>
        ...
        <div className="flex items-center gap-2 mb-6">
          {tabs.map(tab => { const count = counts[tab.key as keyof typeof counts]; const active = activeTab === tab.key; return (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all ${active ? 'bg-[#E7C200] text-black shadow-md shadow-yellow-200' : 'bg-white text-zinc-600 border border-zinc-200 hover:border-[#E7C200] hover:text-[#b09800]'}`}>
              {tab.label}
              {tab.key !== 'all' && <span className={`text-[11px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full ${active ? 'bg-black/10' : 'bg-zinc-100 text-zinc-500'}`}>{count}</span>}
            </button>);
          })}
          <button onClick={load} className="ml-auto text-xs text-zinc-400 hover:text-[#b09800] transition-colors font-medium px-3 py-1.5 rounded-full bg-white border border-zinc-200">↻ Refresh</button>
        </div>
        ...
        : filtered.length === 0 ? <div className="text-center py-20 text-zinc-400">No dining listings in this category</div>
              ...
              <div key={id} className="bg-white rounded-[20px] border border-zinc-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                <div className="flex items-stretch">
                  <div className="w-[100px] flex-shrink-0 bg-zinc-100">
                    ...
                    : <div className="w-full min-h-[120px] flex items-center justify-center text-4xl bg-gradient-to-br from-yellow-50 to-orange-50">🍽️</div>}
                  </div>
                  <div className="flex-1 px-5 py-4 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <h3 className="font-bold text-black text-[15px] leading-snug truncate">{item.name || 'Untitled Restaurant'}</h3>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-zinc-50">
                      <button onClick={() => setPreview(item)} className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200 hover:bg-yellow-100 transition-colors"><Eye size={12}/>Preview</button>
                      <button onClick={() => router.push(`/dining/edit/${id}`)} className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-full bg-indigo-50 text-indigo-600 border border-indigo-200 hover:bg-indigo-100 transition-colors"><Pencil size={12}/>Edit</button>
