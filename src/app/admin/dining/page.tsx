      ...
      <div className="w-full max-w-[680px] bg-white h-full overflow-y-auto shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-7">
          ...
          <div className="grid grid-cols-2 gap-3">
            {([['Date', fmtDate(ev.date)],['Time', ev.time||'—'],['Duration', ev.duration||'—'],['Venue', ev.venue_name||'—'],['Address', ev.venue_address||'—'],['Created', fmtDate(ev.created_at)]] as [string,string][]).map(([l,v]) => (
          ...
          {(ev.google_map_link || ev.instagram_link) && <Section icon={<ExternalLink size={14}/>} title="Links"><div className="flex flex-wrap gap-2">{ev.google_map_link && <LinkChip href={ev.google_map_link} label="Google Maps"/>}{ev.instagram_link && <LinkChip href={ev.instagram_link} label="Instagram"/>}</div></Section>}
