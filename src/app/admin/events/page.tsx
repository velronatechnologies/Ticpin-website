...
function AdminEventDetailView({ ev, onStatus, onUpdate, onDelete, onBack }: {
  ...
  onBack: () => void;
    ...
    // Visual Seating Layout States
    const [isLayoutBased, setIsLayoutBased] = useState(ev.is_layout_based || false);
    const [layoutJson, setLayoutJson] = useState(ev.layout_json || '');
    ...
    // Restore draft & layout data on mount
    useEffect(() => {
        const isLayout = sessionStorage.getItem('is-layout-based') === 'true';
        const returnUrl = sessionStorage.getItem('ticket-editor-return-url') || '';
        const isThisEvent = returnUrl.includes(`id=${id}`);

        if (isLayout && isThisEvent) {
            ...
            if (lCats) {
                try {
                    setTicketCategories(JSON.parse(lCats).map((t: any) => ({
                        ...t,
        ...
        const draftStr = sessionStorage.getItem(`event-admin-edit-draft-${id}`);
        if (draftStr) {
            try {
                ...
                if (draft.secondaryBannerUrl) setSecondaryBannerUrl(draft.secondaryBannerUrl);
                ...
                if (draft.petFriendly) setPetFriendly(draft.petFriendly);
                ...
                console.error('Failed to parse admin event draft:', e);
            }
        }
    }, [id]);
    ...
    const saveDraftToSessionStorage = () => {
        ...
        sessionStorage.setItem(`event-admin-edit-draft-${id}`, JSON.stringify(draft));
    };

    const handleNavigateToLayoutEditor = () => {
        saveDraftToSessionStorage();
        sessionStorage.setItem('ticket-editor-return-url', `/admin/events?id=${id}`);
        sessionStorage.setItem('ticket-layout-json', layoutJson);
        sessionStorage.setItem('is-layout-based', isLayoutBased ? 'true' : 'false');
    ...
    const checkChanges = useCallback(() => {
        const originalData = {
            ...
            is_layout_based: ev.is_layout_based || false,
            layout_json: ev.layout_json || ''
        };
        ...
        selections, pocs, salesNotifs, prohibitedItems, faqs, artists, ticketCategories, isLayoutBased, layoutJson]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Clean up session storage draft for this event
            sessionStorage.removeItem(`event-admin-edit-draft-${id}`);
                ...
                <div className="max-w-[1920px] mx-auto px-10 pt-10">
                    ...
                    <div className="space-y-10 mt-[-30px]">
                        ...
                        <section className="bg-white rounded-[15px] p-8 shadow-sm border border-zinc-100">
                            ...
                            <p className="text-[20px] font-medium text-[#AEAEAE] mb-6">List the specific tickets available or configure an interactive visual seating layout.</p>
                            ...
                            <div className="mb-8 flex gap-3 max-w-[450px] bg-gray-100 p-1 rounded-[12px]">
                                <button
                                    ...
                                    className={`flex-1 py-3 text-[18px] font-semibold rounded-[8px] transition-all duration-200 ${!isLayoutBased ? 'bg-black text-white shadow-sm' : 'text-gray-500 hover:bg-gray-200'} ${!isEditMode ? 'opacity-60 cursor-not-allowed' : ''}`}
                                >
                                    Standard Tickets
                                </button>
                                <button
                                    ...
                                    {!layoutJson ? (
                                        <div className="space-y-6">
                                            ...
                                            {isEditMode ? (
                                                ...
                                                <p className="text-[18px] text-red-500 font-semibold">No seating layout designed for this event.</p>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="space-y-6">
                                            ...
                                            <div className="flex justify-center gap-4 pt-4">
                                                ...
                                                >
                                                    {isEditMode ? 'Edit Venue Map Layout' : 'View Venue Map Layout'}
                                                </button>
                                                {isEditMode && (
                                                    <button
                                                        type="button"
