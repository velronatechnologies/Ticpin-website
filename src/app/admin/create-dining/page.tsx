...
function AdminCreateDiningForm() {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/admin/dining`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                body: JSON.stringify(formData)
            });
            if (response.ok) {
                addToast('Dining outlet created (Admin)!', 'success');
                router.push('/admin');
            } else {
                const data = await response.json();
                addToast(data.message || 'Error creating outlet', 'error');
            }
        } catch (err) { addToast('Error creating outlet', 'error'); }
        ...
        <div className="min-h-screen bg-[#FBFBFF] font-[family-name:var(--font-anek-latin)]">
            <header className="sticky top-0 z-50 bg-white border-b border-zinc-200 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-zinc-900">Admin: Create Dining Outlet</h1>
                    ...
                    <div className="space-y-4">
                        {formData.facilities.map((facility, i) => (
                            <div key={i} className="flex gap-4 items-center">
                                ...
                                <button onClick={() => setFormData({ ...formData, facilities: formData.facilities.filter((_, idx) => idx !== i) })} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors shrink-0"><Trash2 size={20} /></button>
                            </div>
                        ))}
