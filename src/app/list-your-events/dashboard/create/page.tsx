...
import {
    ...
    HelpCircle, FileText, Info, Globe, AlertCircle,
...
export default function CreateEventPage() {
    ...
    useEffect(() => {
        if (editId && token) {
            const fetchEvent = async () => {
                setLoading(true);
                try {
                    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/events/${editId}`, {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: string, index?: number) => {
        ...
        try {
            ...
            if (response.status === 413) {
                addToast('File is too large for the server. Please use a smaller image.', 'error');
                return;
            }
            ...
            if (data.status === 200 && data.data.url) {
                if (target === 'hero') {
                    setFormData(prev => ({ ...prev, images: { ...prev.images, hero: data.data.url } }));
                } else if (target === 'poster') {
                    setFormData(prev => ({ ...prev, images: { ...prev.images, poster: data.data.url } }));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = editId
                ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/events/${editId}`
                : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/api/v1/events`;
    ...
    const handleAutoFill = () => {
        ...
        const now = new Date();
        const start = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000); // 7 days later
        ...
        <div className="min-h-screen bg-[#FBFBFF] font-[family-name:var(--font-anek-latin)]">
            ...
            <header className="sticky top-0 z-50 bg-white border-b border-zinc-200 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-zinc-900">{editId ? 'Edit Event' : 'Create New Event'}</h1>
                        <p className="text-sm text-zinc-500 font-medium">{editId ? 'Update your' : 'Step 1 of 1:'} Event Details</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="px-6 py-2.5 bg-zinc-100 text-zinc-900 font-bold rounded-xl hover:bg-zinc-200 transition-all flex items-center gap-2"
                    >
                        <Plus className="rotate-45" size={18} />
                        Auto Fill (Test)
                    </button>
                    <button onClick={() => router.back()} className="px-6 py-2.5 text-zinc-600 font-bold hover:bg-zinc-100 rounded-xl transition-all">
                        Cancel
                    </button>
                    <button
            ...
            <div className="max-w-[1000px] mx-auto py-12 px-6">
                <form onSubmit={handleSubmit} className="space-y-12">
                    ...
                    <Section icon={<ImageIcon size={20} />} title="Event Creatives">
                        <p className="text-zinc-500 text-sm mb-6 -mt-4">Upload high-quality images to attract more attendees. You need 5 images total (Hero, Poster, and 3 Gallery images).</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <ImageUploadBox
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {formData.images.gallery.map((img, i) => (
                                    <ImageUploadBox
                                        key={i}
                                        value={img}
                                        onUpload={e => handleFileUpload(e, 'gallery', i)}
                                        isUploading={uploading === `gallery${i}`}
                                    />
                                ))}
                            </div>
                        </div>
                    ...
                    <Section icon={<Ticket size={20} />} title="Ticket Inventory">
                        <div className="space-y-6">
                            {formData.tickets.map((ticket, i) => (
                                <div key={i} className="bg-zinc-50 border border-zinc-200 p-6 rounded-3xl relative animate-in fade-in slide-in-from-left-2 transition-all">
                                    <button onClick={() => removeTicket(i)} className="absolute -top-3 -right-3 w-8 h-8 bg-white border border-zinc-200 text-red-500 rounded-full flex items-center justify-center hover:bg-red-50 transition-colors shadow-sm">
                                        <Trash2 size={16} />
                                    </button>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <InputField label="Plan Name" value={ticket.ticket_type} onChange={v => {
                                            const t = [...formData.tickets];
                                            t[i].ticket_type = v;
                                            setFormData({ ...formData, tickets: t });
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                onChange={e => {
                                                    const t = [...formData.tickets];
                                                    t[i].seat_type = e.target.value as 'seating' | 'standing';
                                                    setFormData({ ...formData, tickets: t });
                                                }}
                                                className="w-full h-12 px-4 bg-white border border-zinc-200 rounded-xl text-sm font-medium outline-none border-focus"
                                            >
                                        ...
                                        <InputField label="Total Qty" type="number" value={ticket.total_quantity} onChange={v => {
                                            const t = [...formData.tickets];
                                            t[i].total_quantity = Number(v);
                                            t[i].available_quantity = Number(v);
                                            setFormData({ ...formData, tickets: t });
                                        }} />
                                    </div>
                                </div>
                            ...
                            <button
                                type="button"
                                onClick={addTicket}
                                className="w-full py-6 border-2 border-dashed border-zinc-200 rounded-3xl text-zinc-400 font-bold hover:border-[#5331EA] hover:text-[#5331EA] transition-all flex items-center justify-center gap-2"
                            >
                        ...
                        <div className="space-y-6">
                            {formData.artists.map((artist, i) => (
                                <div key={i} className="bg-zinc-50 border border-zinc-200 p-6 rounded-3xl relative group">
                                    <button onClick={() => removeArtist(i)} className="absolute -top-3 -right-3 w-8 h-8 bg-white border border-zinc-200 text-red-500 rounded-full flex items-center justify-center hover:bg-red-50 transition-colors shadow-sm">
                                        <Trash2 size={16} />
                                    </button>
                                    <div className="flex flex-col md:flex-row gap-8">
                                        <div className="w-32 h-32 shrink-0">
                                            <ImageUploadBox
                                                ...
                                                onUpload={e => handleFileUpload(e, 'artist', i)}
                                                isUploading={uploading === `artist${i}`}
                                            />
                                        </div>
                            ...
                            <button
                                type="button"
                                onClick={addArtist}
                                className="w-full py-4 border-2 border-dashed border-zinc-200 rounded-2xl text-zinc-400 font-bold hover:border-[#5331EA] hover:text-[#5331EA] transition-all flex items-center justify-center gap-2"
                            >
                        ...
                        <div className="space-y-4">
                            {formData.faqs.map((faq, i) => (
                                <div key={i} className="flex gap-4 items-start">
                                    ...
                                    <button onClick={() => removeFaq(i)} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors shrink-0">
                                        <Trash2 size={20} />
                                    </button>
                                </div>
                            ))}
                            <button type="button" onClick={addFaq} className="text-[#5331EA] font-bold flex items-center gap-2 text-sm">
                            ...
                            {formData.terms_and_conditions.map((term, i) => (
                                <div key={i} className="flex gap-4 items-center">
                                    <input
                                        value={term}
                                        onChange={e => {
                                            const t = [...formData.terms_and_conditions];
                                            t[i] = e.target.value;
                                            setFormData({ ...formData, terms_and_conditions: t });
                                        }}
                                        placeholder="Term..."
                                        className="flex-1 h-12 px-4 bg-white border border-zinc-200 rounded-xl text-sm font-medium outline-none"
                                    />
                                    <button onClick={() => removeTerm(i)} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors shrink-0">
                                        <Trash2 size={20} />
                                    </button>
                                </div>
                            ))}
                            <button type="button" onClick={addTerm} className="text-[#5331EA] font-bold flex items-center gap-2 text-sm">
