...
function CreatePlayVenueForm() {
    ...
    const hasPlayAccess = organizerCategories.includes('play');
    ...
    useEffect(() => {
        ...
        if (!hasPlayAccess) {
            addToast('Access restricted. You are not verified for Play/Courts.', 'error');
            router.push('/organizer-dashboard?category=play');
        }
    }, [token, hasPlayAccess, router]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                addToast(editId ? 'Venue updated!' : 'Venue created and submitted for review!', 'success');
                router.push('/organizer-dashboard?category=play');
            } else {
                const data = await response.json();
                addToast(data.message || 'Error saving venue', 'error');
            }
        } catch (err) { addToast('Error saving venue', 'error'); }
