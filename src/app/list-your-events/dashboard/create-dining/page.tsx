...
function CreateDiningOutletForm() {
    ...
    const hasDiningAccess = organizerCategories.includes('dining');
    ...
    useEffect(() => {
        ...
        if (!hasDiningAccess) {
            addToast('Access restricted. You are not verified for Dining.', 'error');
            router.push('/organizer-dashboard?category=dining');
        }
    }, [token, hasDiningAccess, router]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                addToast(editId ? 'Outlet updated!' : 'Outlet created and submitted for review!', 'success');
                router.push('/organizer-dashboard?category=dining');
            } else {
                const data = await response.json();
        ...
        <div className="min-h-screen bg-[#FBFBFF] font-[family-name:var(--font-anek-latin)]">
            <header className="sticky top-0 z-50 bg-white border-b border-zinc-200 px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        {loading ? 'Saving...' : (editId ? 'Save Changes' : 'Submit for Review')}
            ...
            <div className="max-w-[1000px] mx-auto py-12 px-6 space-y-12">
                ...
                <Section icon={<Clock size={20} />} title="Booking Settings">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <InputField label="Advance Booking Days" type="number" value={formData.booking_settings.advance_booking_days} onChange={v => setFormData({ ...formData, booking_settings: { ...formData.booking_settings, advance_booking_days: Number(v) } })} />
                        <InputField label="Avg Duration (min)" type="number" value={formData.booking_settings.average_dining_duration_minutes} onChange={v => setFormData({ ...formData, booking_settings: { ...formData.booking_settings, average_dining_duration_minutes: Number(v) } })} />
                    </div>
                </Section>
                    ...
                    <div className="space-y-6">
                        {formData.offers.map((offer, i) => (
                            <div key={i} className="p-6 bg-zinc-50 border border-zinc-200 rounded-3xl grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                                ...
                                <ImageUploadBox label="Offer Image (poster)" value={(offer as any).offer_image || ''} onUpload={e => handleFileUpload(e, 'offer_image', i)} isUploading={uploading === `offer_image${i}`} />
                                {i > 0 && (
                    ...
                    <div className="space-y-4">
                        {formData.facilities.map((facility, i) => (
                            <div key={i} className="flex gap-4 items-center">
                                <input value={facility} onChange={e => { const f = [...formData.facilities]; f[i] = e.target.value; setFormData({ ...formData, facilities: f }); }} placeholder="Facility..." className="flex-1 h-12 px-4 bg-white border border-zinc-200 rounded-xl text-[15px] font-medium outline-none border focus:border-black transition-all" />
                                <button type="button" onClick={() => setFormData({ ...formData, facilities: formData.facilities.filter((_, idx) => idx !== i) })} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors shrink-0"><Trash2 size={20} /></button>
                            </div>
                        ))}
                        ...
                        {formData.faqs.map((faq, i) => (
                            <div key={i} className="flex gap-4 items-start">
                                <div className="flex-1 space-y-4">
                                    <input value={faq.question} onChange={e => { const f = [...formData.faqs]; f[i].question = e.target.value; setFormData({ ...formData, faqs: f }); }} placeholder="Question" className="w-full h-12 px-4 bg-white border border-zinc-200 rounded-xl text-[15px] font-medium outline-none border focus:border-black transition-all" />
                                    <input value={faq.answer} onChange={e => { const f = [...formData.faqs]; f[i].answer = e.target.value; setFormData({ ...formData, faqs: f }); }} placeholder="Answer" className="w-full h-12 px-4 bg-white border border-zinc-200 rounded-xl text-[15px] font-medium outline-none border focus:border-black transition-all" />
                                </div>
                ...
                <Section icon={<Info size={20} />} title="Terms & Conditions">
                    <div className="space-y-4">
                        {formData.terms_and_conditions.map((term, i) => (
                            <div key={i} className="flex gap-4 items-center">
                                <input value={term} onChange={e => { const t = [...formData.terms_and_conditions]; t[i] = e.target.value; setFormData({ ...formData, terms_and_conditions: t }); }} placeholder="Term..." className="flex-1 h-12 px-4 bg-white border border-zinc-200 rounded-xl text-[15px] font-medium outline-none border focus:border-black transition-all" />
