...
export default function LoginPage() {
    ...
    useEffect(() => {
        ...
        if (session) {
            if (session.isAdmin) {
                ...
                router.replace(session.categoryStatus?.events
                    ? '/organizer/dashboard?category=events'
                    : '/list-your-events/setup');
            }
        }
    }, [router]);
    ...
    const handleLogin = async () => {
        ...
        try {
            await eventsApi.login(email, password);
            sessionStorage.setItem('otp_pending_email', email);
            router.push('/list-your-events/otp');
        } catch (e: unknown) {
            const msg = e instanceof Error ? e.message : 'Login failed';
            if (msg === 'user_not_found') {
                router.push(`/list-your-events/Signin?email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`);
            } else {
            ...
            <main className="flex-1 flex flex-col items-center justify-start pt-20 px-6 overflow-y-auto scrollbar-hide">
                <div className="w-full max-w-[1000px]">
                    ...
                    <p className="text-base font-medium text-black mb-12" style={{ fontSize: '15px', lineHeight: '16px' }}>
                        ...
                        <Link href="/list-your-events/Signin" className="text-[#5331EA] border-b border-[#5331EA] hover:opacity-80">Sign up</Link>
                    </p>
