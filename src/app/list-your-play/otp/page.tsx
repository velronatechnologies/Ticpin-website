...
function PlayOTPContent() {
    ...
    const handleVerify = async () => {
        ...
        try {
            const res = await playApi.verifyOTP(email, code);
            const id = (res._id ?? res.id ?? '').toString();
            const catStatus = res.categoryStatus ?? {};
            const isAdmin = !!res.isAdmin;
            saveOrganizerSession({ id, email, vertical: 'play', isAdmin, categoryStatus: catStatus });
            sessionStorage.removeItem('otp_pending_email');

            if (isAdmin) {
                router.push('/admin');
            } else if (catStatus['play']) {
    ...
    return (
        <div className="overflow-hidden flex flex-col font-[family-name:var(--font-anek-latin)]"
            style={{ background: 'rgba(255, 241, 168, 0.1)', height: 'calc(100vh - 80px)' }}>
