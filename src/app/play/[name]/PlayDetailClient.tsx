                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-base font-semibold text-black">
                                                {(() => {
                                                    // Priority 1: pricing_plans (fixed format)
                                                    if (venue.pricing_plans && venue.pricing_plans.length > 0) {
                                                        const plan = venue.pricing_plans[0];
                                                        return `${toDisplayTime(plan.start_time)} - ${toDisplayTime(plan.end_time)}`;
                                                    }
                                                    // Priority 2: opening_time and closing_time
                                                    if (venue.opening_time?.trim().replace(/-/g, '') && venue.closing_time?.trim().replace(/-/g, '')) {
                                                        return `${toDisplayTime(venue.opening_time)} - ${toDisplayTime(venue.closing_time)}`;
                                                    }
                                                    // Priority 3: time field
                                                    if (venue.time?.trim().replace(/-/g, '')) {
                                                        return toDisplayTime(venue.time);
                                                    }
                                                    return 'N/A';
                                                })()}
                                            </p>
