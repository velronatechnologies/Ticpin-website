...
export default function PickleballPage() {
    ...
    useEffect(() => {
        const fetchVenues = async () => {
            setIsLoading(true);
            try {
                const response = await playApi.getAll(20, '', 'Pickleball');
            ...
            <main className="space-y-12 md:space-y-20">
                ...
                <section className="relative w-full h-[300px] md:h-[400px] flex items-center justify-center overflow-hidden"
                    ...
                    <div className="flex items-center gap-8 md:gap-12 px-4">
                        ...
                        <h1 className="text-4xl md:text-7xl font-semibold text-black uppercase leading-tight">
                            Pickleball
                        </h1>
                    </div>
                </section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 max-w-7xl mx-auto">
                            {venueList.length > 0 ? (
                                ...
                                <div className="col-span-full text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
                                    <p className="text-gray-500 text-lg">No pickleball venues available right now. Check back soon!</p>
                                </div>
                            )}
                        </div>
                    )}
