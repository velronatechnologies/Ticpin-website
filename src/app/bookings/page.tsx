                        ...
                        filteredBookings.map((booking, idx) => (
                            <div key={booking.id || idx} className="bg-white border-[0.5px] border-[#686868] rounded-[25px] px-[35px] pt-[35px] pb-[20px] relative">
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="w-[134px] h-[75px] bg-zinc-100 rounded-[15px] overflow-hidden flex items-center justify-center border border-zinc-200">
                                ...
                                <div className="mt-[18px] pt-3 border-t border-[#D1D1D1] flex justify-between items-center">
                                    <div className={`px-4 py-1.5 rounded-[9px] flex items-center justify-center min-w-[104px] ${
                                        (booking.status === 'booked' || booking.status === 'confirmed' || booking.status === 'pending') ? 'bg-[#D6FAE5]' : 
                                        'bg-red-50'
                                    }`}>
                                        <span className={`text-[15px] font-bold uppercase tracking-wider ${
                                            (booking.status === 'booked' || booking.status === 'confirmed' || booking.status === 'pending') ? 'text-[#009133]' : 
                                            'text-red-600'
                                        }`}>
                                            {(booking.status === 'booked' || booking.status === 'confirmed' || booking.status === 'pending') ? 'Confirmed' : booking.status || 'BOOKED'}
                                        </span>
                                    </div>
