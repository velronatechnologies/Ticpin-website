...
export default function ChatSupportClient() {
    ...
    // Admin check: relies on isAdmin flag from server-issued JWT — never a hardcoded phone number
    const isAdmin = (organizerSession?.isAdmin === true) || (userSession as any)?.isAdmin === true;
        ...
        return (
            <div className="min-h-screen bg-white flex flex-col items-center">
                ...
                <main className="max-w-[1440px] w-full pt-[25px] px-4 flex flex-col items-center">
                    <h1 className="text-[28px] md:text-[34px] font-semibold text-black mb-[30px] md:mb-[40px]">Chat with us</h1>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-[40px] lg:gap-[100px] mb-10 md:mb-[100px]">
                        {supportOptions.map((option) => (
                            <div
                                ...
                                className={`w-full sm:w-[193px] aspect-square sm:h-[210px] rounded-[30px] md:rounded-[40px] flex flex-col items-center justify-center transition-colors ${isAdmin ? 'bg-[#E1E1E1] cursor-pointer hover:bg-zinc-200' :
                            ...
                            >
                                <div className="mb-3 md:mb-4">
                                    <Image src={option.icon} alt={option.title} width={option.width * 0.8} height={option.height * 0.8} className="object-contain w-10 h-10 md:w-auto md:h-auto" />
                                </div>
                                <span className="text-[16px] md:text-[25px] font-medium text-black text-center px-4 leading-tight">{option.title}</span>
    ...
    return (
        <div className="min-h-screen bg-white flex flex-col items-center">
            ...
            <main className="max-w-[1440px] w-full h-[calc(100vh-75px)] p-4 md:p-10 flex flex-col items-center">
                <h1 className="text-[24px] md:text-[34px] font-semibold text-black mb-4 md:mb-6">Chat with us</h1>
                <div className="text-lg md:text-xl font-medium text-[#5331EA] mb-4">Today</div>

                <div className="w-full max-w-[1363px] flex-1 md:h-[510px] bg-[#F0F0F0] rounded-[20px] md:rounded-[30px] relative overflow-hidden flex flex-col shadow-inner">
                    <div className="flex-1 p-5 md:p-10 overflow-y-auto space-y-4 md:space-y-6 scrollbar-hide">
                        {Array.isArray(messages) && messages.map((msg, idx) => (
                            <div key={idx} className={`flex items-start gap-2 md:gap-4 ${msg.sender === 'admin' ? 'justify-start' : 'justify-end'}`}>
                                {msg.sender === 'admin' && (
                                    <div className="w-[30px] h-[30px] md:w-[38px] md:h-[38px] bg-[#E5E0FC] rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
                                        <Image src="/admin panel/chatSupport/Ellipse 29.svg" alt="Support" width={38} height={38} />
                                    </div>
                                )}
                                <div className={`max-w-[85%] md:max-w-[70%] flex flex-col ${msg.sender === 'admin' ? 'items-start' : 'items-end'}`}>
                                    <div className={`p-3 md:p-4 rounded-2xl relative ${msg.sender === 'admin' ? 'bg-[#D8D3EF] text-black rounded-tl-none' : 'bg-[#5331EA] text-white rounded-tr-none shadow-md'}`}>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-[10px] md:text-[12px] font-bold">{msg.sender === 'admin' ? 'Ticpin' : 'You'}</span>
                                            {msg.sender === 'admin' && <span className="text-[8px] opacity-60">Assistant</span>}
                                        </div>
                                        {msg.message && <p className="text-xs md:text-sm mb-2 leading-relaxed">{msg.message}</p>}
                    ...
                    <div className="p-4 md:p-8 md:pb-10 flex items-center gap-3 md:gap-5 justify-center bg-zinc-50/50 border-t border-zinc-200">
                            ...
                            <>
                                ...
                                <div className="flex items-center gap-2 md:gap-3">
                                    ...
                                    <button
                                        ...
                                        className={`cursor-pointer transition-all ${isListening ? 'text-red-500 animate-pulse' : 'text-zinc-400 hover:text-[#5331EA]'}`}
                                        title="Voice message"
                                    >
                                ...
                                <input
                                    ...
                                    className="hidden"
                                />
                                <div className="flex-1 max-w-[1080px] h-[44px] md:h-[50px] bg-white border border-[#5331EA] rounded-full px-4 md:px-6 flex items-center shadow-sm">
                                    <input
                                        type="text"
                                        value={inputValue}
                                        ...
                                        className="w-full bg-transparent border-none outline-none text-[14px] md:text-[15px] text-black"
                                    />
                                </div>
                                <button
                                    onClick={handleSendMessage}
                                    disabled={(!inputValue.trim() && attachedFiles.length === 0) || uploadingFiles}
                                    className="w-[44px] h-[44px] md:w-[50px] md:h-[50px] bg-[#5331EA] text-white rounded-full flex items-center justify-center transition-all hover:bg-[#4227c0] active:scale-95 disabled:opacity-50 flex-shrink-0"
                                >
                                    {uploadingFiles ? (
