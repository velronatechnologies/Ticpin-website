...
export default function BuyPassPage() {
    ...
    const handlePayment = async () => {
        ...
        // Check if email already exists in database BEFORE payment
        try {
            const checkRes = await fetch(`/backend/api/profiles/check-email?email=${encodeURIComponent(formData.email)}`);
            const checkData = await checkRes.json();
            
            if (checkData.exists) {
                // If it's an organizer account
                if (checkData.isOrganizer) {
                    toast.error('This email is already associated with an organizer account. Please use a different email.');
                    return;
                }
                // If it's another user account
                if (checkData.isUser && user?.id && checkData.userId !== user.id) {
