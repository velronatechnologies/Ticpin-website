...
export default function FamilyFavouritesPage() {
    ...
    useEffect(() => {
        const fetchRestaurants = async () => {
            setIsLoading(true);
            try {
                const response = await diningApi.getAll(20, '', 'Family Favourites');
                if (response.success && response.data) {
                    setRestaurantList(response.data.items || []);
                }
            } catch (error) {
                console.error("Failed to fetch family favourites venues:", error);
            } finally {
                setIsLoading(false);
            }
        };
