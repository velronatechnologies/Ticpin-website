...
export default function DiningReviewPage() {
    ...
    const handleContinue = () => {
        if (!isMobile && (!billing.phone || billing.phone.length < 10)) {
    ...
    const handlePayNow = async () => {
        ...
        try {
            const emailCheckRes = await fetch(`/backend/api/profiles/check-email?email=${encodeURIComponent(billing.email)}`, {
                credentials: 'include',
            });
            if (emailCheckRes.ok) {
            ...
            const options = {
                ...
                handler: async (response: any) => {
                    await completeDiningBooking(
                        ...
                        'razorpay',
                        response.razorpay_order_id || res.order_id
                    );
                },
                prefill: {
                    name: billing.name,
    ...
    const completeDiningBooking = async (paymentId: string, gateway: string, orderId?: string) => {
        try {
    ...
    if (isMobile) {
        return (
            <MobileDiningReview
                ...
                email={billing.email}
                setEmail={(val) => setBilling((b: any) => ({ ...b, email: val }))}
