...
export default function BarBitesPage() {
    ...
    useEffect(() => {
        const fetchRestaurants = async () => {
            setIsLoading(true);
            try {
                // Category name matches the h1 title usually or what's seeded
                const response = await diningApi.getAll(20, '', 'Bar & Bites');
                if (response.success && response.data) {
                    setRestaurantList(response.data.items || []);
                }
            } catch (error) {
                console.error("Failed to fetch bar bites venues:", error);
            } finally {
                setIsLoading(false);
            }
        };
