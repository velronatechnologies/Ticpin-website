...
function ProfileContent() {
    ...
    return (
        <div className={`min-h-screen flex flex-col font-[family-name:var(--font-anek-latin)] pb-24 ${isPlayVerified ? 'bg-gradient-to-b from-[#FFFCED] via-white' : 'bg-[#5331EA]/5'}`}>
            ...
            <main className="flex-1 w-full max-w-[1100px] mx-auto px-6 py-8 space-y-8">
                ...
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        <h1 className="text-[24px] md:text-[28px] font-bold text-black leading-tight">Settings</h1>
                        <p className="text-[15px] md:text-[16px] font-medium text-[#686868]">Manage your brand settings here</p>
                    </div>
                    {!isEditing && (
                        <button
                            onClick={() => setIsEditing(true)}
                            className="px-6 py-2 bg-black text-white rounded-lg font-bold text-sm md:text-base hover:bg-zinc-800 transition-all active:scale-95 shadow-md"
                        >
                            Edit
                        </button>
                    )}
                ...
                {/* Login Details Card */}
                <div className="bg-white rounded-[12px] p-6 space-y-6 shadow-sm border border-zinc-100">
                    <h2 className="text-[18px] md:text-[20px] font-semibold text-black flex items-center gap-1">
                        Login details<span className="text-red-500">*</span>
                    </h2>

                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <label className="text-[14px] md:text-[15px] font-semibold text-black w-36">User email:</label>
                        <div className="flex-1 max-w-[800px] flex items-center gap-3">
                            <div className="flex-1 h-[42px] md:h-[46px] bg-[#F3F4F6] border border-[#D1D5DB] rounded-[6px] flex items-center px-4">
                                <span className="text-[14px] md:text-[15px] font-medium text-zinc-700">{formData.email}</span>
                            </div>
                            {isEditing && (
                                <button
                                    onClick={() => setIsEmailModalOpen(true)}
                                    className="px-4 h-[42px] md:h-[46px] bg-black text-white rounded-[6px] font-bold text-sm hover:bg-zinc-800 transition-all active:scale-95 shrink-0"
                                >
                                    Change
                                </button>
                            )}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <label className="text-[14px] md:text-[15px] font-semibold text-black w-36">User phone:</label>
                        <div className="flex-1 max-w-[800px] flex items-center gap-3">
                            <div className="flex-1 h-[42px] md:h-[46px] bg-[#F3F4F6] border border-[#D1D5DB] rounded-[6px] flex items-center px-4">
                                <span className="text-[14px] md:text-[15px] font-medium text-zinc-700">{formData.phone || 'Not specified'}</span>
                            </div>
                            {isEditing && (
                                <button
                ...
                <div className="bg-white rounded-[12px] p-6 space-y-6 shadow-sm border border-zinc-100">
                    <div className="space-y-1">
                        ...
                        <p className="text-[14px] md:text-[15px] font-medium text-[#686868]">Add brand details to display across your hosted events.</p>
                    </div>

                    <div className="w-full h-[1px] bg-[#E5E7EB]" />

                    <div className="space-y-4">
                        <p className="text-[14px] md:text-[15px] font-medium text-[#686868]">Select your brand from the dropdown below and add their profile details.</p>

                        <div className="flex flex-col md:flex-row md:items-center gap-4">
                            <label className="text-[14px] md:text-[15px] font-semibold text-black w-36">Select your brand:</label>
                            <div className="flex-1 max-w-[800px] h-[42px] md:h-[46px] bg-[#F3F4F6] border border-[#D1D5DB] rounded-[6px] flex items-center justify-between px-4 cursor-pointer">
                                <span className="text-[14px] md:text-[15px] font-medium text-zinc-700">{formData.brandName || '{BRAND NAME}'}</span>
                                <ChevronDown className="text-[#686868]" size={20} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="space-y-1">
                            <h3 className="text-[18px] md:text-[20px] font-semibold text-black uppercase">{formData.brandName || '{CHOOSED BRAND NAME}'}</h3>
                        </div>

                        <div className="flex flex-col md:flex-row gap-8">
                            {/* Photo Upload */}
                            <div className="flex flex-col items-center gap-3 w-[150px] shrink-0">
                                <div
                                    className={`w-[110px] h-[110px] rounded-full border-[2px] border-[#E7C200] flex flex-col items-center justify-center cursor-pointer overflow-hidden relative group bg-white ${!isEditing ? 'pointer-events-none' : ''}`}
                                ...
                                >
                                    {previewPhoto ? (
                                        ...
                                        <div className="flex flex-col items-center gap-1.5">
                                            ...
                                            <span className="text-[14px] font-semibold text-[#E7C200]">Upload</span>
                                        </div>
                                    )}
                                ...
                                <p className="text-[12px] font-medium text-[#686868] text-center">This photo is displayed on your Organiser page.</p>
                            </div>

                            {/* Description Input */}
                            <div className="flex-1 max-w-[800px]">
                                <textarea
                                    className={`w-full h-[140px] border border-[#D1D5DB] rounded-[6px] p-4 text-[14px] md:text-[15px] outline-none focus:ring-1 focus:ring-black ${!isEditing ? 'bg-[#F9FAFB] text-[#686868]' : 'bg-white'}`}
            ...
            {/* Save Button Footer */}
            {isEditing && (
                <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md flex flex-col items-center justify-center px-6 z-50 border-t border-zinc-200 py-3 animate-in slide-in-from-bottom duration-300">
                    {saveError && (
                        <div className="w-full max-w-[1100px] p-2 bg-red-50 text-red-600 rounded-t-lg text-center text-xs font-medium border-b border-red-100 mb-2">
                            {saveError}
                        </div>
                    )}
                    <div className="w-full max-w-[1100px] flex gap-3">
                        <button
                            onClick={() => {
                                setIsEditing(false);
                                setSaveError('');
                            }}
                            className="flex-1 h-[42px] md:h-[46px] bg-zinc-200 text-black rounded-[8px] text-[15px] md:text-[16px] font-semibold flex items-center justify-center transition-all active:scale-[0.98] hover:bg-zinc-300"
                        >
                            Cancel
                        </button>
                        <button
                            onClick={handleSave}
                            disabled={saving}
                            className="flex-[2] h-[42px] md:h-[46px] bg-black text-white rounded-[8px] text-[15px] md:text-[16px] font-semibold flex items-center justify-center gap-2 transition-all active:scale-[0.98] hover:bg-zinc-900 disabled:opacity-50 shadow-md"
                        >
