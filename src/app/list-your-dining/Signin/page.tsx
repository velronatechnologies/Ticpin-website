...
export default function SignupPage() {
    ...
    // Prefill email/password if redirected from Login with user_not_found
    useEffect(() => {
    ...
    const handleSignup = async () => {
        ...
        try {
            await diningApi.signin(email, password);
            router.push(`/list-your-dining/otp?email=${encodeURIComponent(email)}`);
        } catch (e: unknown) {
            ...
            <main className="flex-1 flex flex-col items-center justify-start pt-20 px-6 overflow-y-auto scrollbar-hide">
                <div className="w-full max-w-[1000px]">
                    ...
                    <p className="text-base font-medium text-black mb-12" style={{ fontSize: '15px', lineHeight: '16px' }}>
                        ...
                        <Link href="/list-your-dining/Login" className="text-[#5331EA] border-b border-[#5331EA] hover:opacity-80">Log in</Link>
                    </p>
                    <div className="grid grid-cols-2 gap-8 mb-8">
                        ...
                        <div>
                            ...
                            <input type="password" placeholder="Password (min 8 chars)" value={password} onChange={e => setPassword(e.target.value)}
                                ...
                                className="w-full px-6 py-4 border-[1.5px] border-[#AEAEAE] rounded-[20px] text-[#AEAEAE] placeholder-[#AEAEAE] focus:outline-none focus:border-black transition-colors"
                                style={{ height: '65px' }} />
                            <p className="text-sm font-medium text-[#5331EA] mt-4" style={{ fontSize: '13px', lineHeight: '16px' }}>
                    ...
                    {error && (
                        <div className="mb-4">
                            ...
                            {error.includes('already exists') && (
                                <Link href="/list-your-dining/Login" className="text-[#5331EA] text-sm underline mt-1 block">Go to Login →</Link>
                            )}
                        </div>
                    )}
