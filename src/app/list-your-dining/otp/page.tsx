...
function DiningOTPContent() {
    ...
    const handleVerify = async () => {
        ...
        try {
            const res = await diningApi.verifyOTP(email, code);
            const id = (res._id ?? res.id ?? '').toString();
            const catStatus = res.categoryStatus ?? {};
            const isAdmin = !!res.isAdmin;
            saveOrganizerSession({ id, email, vertical: 'dining', isAdmin, categoryStatus: catStatus });
            sessionStorage.removeItem('otp_pending_email');

            if (isAdmin) {
                router.push('/admin');
            } else if (catStatus['dining']) {
    ...
    const handleResend = async () => {
        ...
        try {
            await diningApi.resendOTP(email);
            setResent(true);
        } catch {
