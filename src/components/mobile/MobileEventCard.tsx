...
export default function MobileEventCard({
    ...
    }, [id, name, date, time, price_starts_from, portrait_image_url, venue_name, city, session?.id, onUnlike]);
    ...
    return (
        ...
        >
            ...
            <div className="p-3.5 relative bg-white">
                ...
                <button
                    onClick={toggleLike}
                    className={`absolute top-3 right-3 w-8 h-8 rounded-[8px] flex items-center justify-center transition-colors cursor-pointer z-10 active:scale-90 ${isLiked ? 'bg-red-500' : 'bg-[#EFEFEF]'}`}
                >
                    <img
