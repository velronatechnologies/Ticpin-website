...
export default function MobileEventCard({
    ...
    }, [id, name, date, time, price_starts_from, portrait_image_url, venue_name, city]);
    ...
    return (
        ...
        >
            ...
            <div className="p-5 relative bg-white">
                ...
                <h3 className={`text-[20px] font-medium uppercase leading-[1.1] tracking-tight line-clamp-2 transition-colors ${isLiked ? 'text-red-500' : 'text-black'}`}>
                    {name || 'Event Name'}
                </h3>
                ...
                <div className={`absolute top-5 right-5 w-10 h-10 rounded-[12px] flex items-center justify-center mt-[-10px] transition-colors ${isLiked ? 'bg-red-500' : 'bg-[#EFEFEF]'}`}>
                    <img
