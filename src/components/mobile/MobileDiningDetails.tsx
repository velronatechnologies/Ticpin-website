...
export default function MobileDiningDetails({ venue, offers }: MobileDiningDetailsProps) {
    ...
    useEffect(() => {
        ...
        // For dining, we use the specific dining like API
        if (session?.id) {
            fetch(`/backend/api/user/likes/dining/${venue.id}`, { credentials: 'include' })
                .then(res => res.ok ? res.json() : Promise.reject())
                .then(data => setIsLiked(data.liked))
                .catch(() => {
                    const liked = JSON.parse(localStorage.getItem('liked_dining') || '[]');
                    setIsLiked(liked.some((e: any) => e.id === venue.id));
                });
        } else {
            const liked = JSON.parse(localStorage.getItem('liked_dining') || '[]');
            setIsLiked(liked.some((e: any) => e.id === venue.id));
    ...
    const handleLikeToggle = async () => {
        ...
        if (!session?.id) {
            ...
            let liked = JSON.parse(localStorage.getItem('liked_dining') || '[]');
            const isAlreadyLiked = liked.some((e: any) => e.id === venue.id);
            if (isAlreadyLiked) {
                liked = liked.filter((e: any) => e.id !== venue.id);
                setIsLiked(false);
                toast.success('Removed from Ticlists');
            } else {
                liked.push({
                    id: venue.id,
                    name: venue.name,
                    portrait_image_url: venue.portrait_image_url,
                    landscape_image_url: venue.landscape_image_url,
                    city: venue.city,
                    rating: venue.rating
                });
                setIsLiked(true);
            ...
            localStorage.setItem('liked_dining', JSON.stringify(liked));
            return;
        }

        try {
            const res = await fetch(`/backend/api/user/likes/dining/${venue.id}`, {
                method: 'POST',
                credentials: 'include'
            });
            if (res.ok) {
                ...
                toast.success(data.liked ? 'Saved to Ticlists' : 'Removed from Ticlists');
            }
        } catch (error) {
