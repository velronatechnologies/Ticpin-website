...
export default function MobilePlayDetails({ venue, offers = [] }: MobilePlayDetailsProps) {
    ...
    useEffect(() => {
        ...
        if (session?.id) {
            fetch(`/backend/api/user/likes/play/${venue.id}`, { credentials: 'include' })
                .then(res => res.ok ? res.json() : Promise.reject())
                .then(data => setIsLiked(data.liked))
                .catch(() => {
                    const liked = JSON.parse(localStorage.getItem('liked_play') || '[]');
    ...
    const handleLikeToggle = async () => {
        ...
        if (!session?.id) {
            let liked = JSON.parse(localStorage.getItem('liked_play') || '[]');
        ...
        try {
            const res = await fetch(`/backend/api/user/likes/play/${venue.id}`, {
                method: 'POST',
                credentials: 'include'
            });
            if (res.ok) {
