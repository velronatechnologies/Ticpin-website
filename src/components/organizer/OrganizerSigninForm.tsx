...
import { saveOrganizerSession, getOrganizerSession } from '@/lib/auth/organizer';
...
export default function OrganizerSigninForm({ vertical, setupPath, loginPath, signinTitle }: Props) {
    ...
    useEffect(() => {
        ...
        if (e) {
            setIdentifier(decodeURIComponent(e));
            if (!decodeURIComponent(e).includes('@')) setLoginType('mobile');
    ...
    const handleSignup = async () => {
        ...
        try {
            ...
            setError(msg === 'email_exists' ? 'An account with this identifier already exists' : msg);
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        ...
        if (userSession) {
            toast.error('Please logout from your user account first', 5000);
            router.push('/');
        }
    }, [router]);
            ...
            <main className="flex-1 flex flex-col items-center justify-start pt-20 px-6 overflow-y-auto scrollbar-hide">
                <div className="w-full max-w-[1000px]">
                    ...
                    <h2 className="text-3xl font-medium text-black mb-4" style={{ fontSize: '30px', lineHeight: '33px', marginBottom: '24px' }}>
                        {signinTitle || 'Sign up Business account'}
                    </h2>
