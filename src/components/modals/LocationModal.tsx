...
const ICONS: Record<string, string> = {
    ...
    coimbatore: "M15 50h30 M20 50V25h20v25 M20 25c0-10 20-10 20 0 M30 35m-5 0a5 5 0 1 1 10 0a5 5 0 1 1 -10 0 M30 32v3h3",
    kochi: "M15 45h30l-5 5H20z M30 45V20 M30 20c8 10 8 20 0 25 M10 52c5-2 10 0 15-2s10 2 15-2 10 2 15-2",
...
export default function LocationModal({ isOpen, onClose, onSelect }: LocationModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-[family-name:var(--font-anek-tamil)]">
            <div className="w-full max-w-[780px] bg-white rounded-[20px] shadow-2xl animate-in fade-in zoom-in duration-300 relative flex flex-col max-h-[85vh] overflow-hidden">

                {/* Header */}
                <div className="p-4 md:p-6 pb-3 flex-shrink-0">
                    <button
                        onClick={onClose}
                        className="absolute right-4 top-4 md:right-6 md:top-6 p-2 rounded-full text-zinc-400"
                    >
                        <X size={20} />
                    </button>
                    <h2 className="text-[15px] md:text-[16px] font-semibold text-zinc-800 mb-3 md:mb-4">Select Location</h2>
                    ...
                    <div className="relative mb-2.5 md:mb-4">
                        <input
                            ...
                            className="w-full h-[40px] md:h-[48px] px-3 md:px-4 border border-zinc-200 rounded-[10px] text-[13px] md:text-[14px] focus:outline-none focus:border-[#9d7fce] focus:ring-4 focus:ring-[#9d7fce]/10"
                        />
                    </div>

                    {/* Location Buttons */}
                    <div className="flex items-center gap-3.5 flex-wrap mb-2.5 md:mb-4">
                        <button
                            onClick={handleCurrentLocation}
                            className="flex items-center gap-2 text-[#9d7fce] font-semibold text-[13px] md:text-[14px]"
                        >
                            <MapPin size={18} />
                        ...
                        {currentLocation && (
                            <button
                                ...
                                className="flex items-center gap-1.5 text-rose-600 font-semibold text-[13px] md:text-[14px] border-l border-zinc-200 pl-3.5"
                            >
                                <X size={14} />
                                Remove Location
                            </button>
                    ...
                    {/* Loader */}
                    {isLoading && (
                        <div className="flex justify-start mb-2.5 md:mb-4 items-center">
                            <Loader2 className="w-5 h-5 text-[#9d7fce] animate-spin" />
                            <span className="ml-2 text-zinc-500 text-xs">Detecting location...</span>
                        </div>
                    )}
                </div>

                {/* Scrollable Content */}
                <div className="overflow-y-auto px-4 md:px-6 pb-4 md:pb-6 flex-1 space-y-4 md:space-y-8 [scrollbar-width:none] [::-webkit-scrollbar]:hidden">
                    {!showSearchResults ? (
                        <>
                            {/* Popular */}
                            <div>
                                <h3 className="text-[14px] md:text-[15px] font-semibold text-zinc-800 mb-2.5 md:mb-4">Popular Cities</h3>
                                <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 md:gap-3">
                                    {popularCities.map(city => (
                                        <button
                                            key={city.name}
                                            onClick={() => handleSelect(city.name)}
                                            className="flex flex-col items-center justify-center p-2 md:p-3.5 bg-zinc-50 border border-transparent rounded-[10px] md:rounded-[12px] group"
                                        >
                                            <svg className="w-[28px] h-[28px] md:w-[48px] md:h-[48px] mb-1 md:mb-2 text-zinc-500" viewBox="0 0 60 60">
                                                <path fill="none" stroke="currentColor" strokeWidth="1.5" d={ICONS[city.icon]} />
                                            </svg>
                                            <span className="text-[10px] md:text-[12px] font-semibold text-zinc-800 text-center truncate w-full">{city.name}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="space-y-3 md:space-y-4">
                                <h3 className="text-[14px] md:text-[15px] font-semibold text-zinc-800">All Cities</h3>
                                <div className="flex flex-wrap gap-1 md:gap-2">
                                    {"ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map(l => (
                                        <button
                                            key={l}
                                            onClick={() => setActiveLetter(l)}
                                            className={`text-[11px] md:text-[12px] font-semibold ${activeLetter === l ? 'text-[#9d7fce]' : 'text-zinc-300'}`}
                                        >
                                            {l}
                                        </button>
                                    ))}
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-y-1 md:gap-y-1.5 gap-x-3 md:gap-x-4">
                                    {filteredCities.map((city, idx) => (
                                        <button
                                            key={`${city}-${idx}`}
                                            onClick={() => handleSelect(city)}
                                            className="text-left text-[11px] md:text-[13px] font-medium text-zinc-600 py-0.5 md:py-1 truncate"
                                        >
                                            {city}
                                        </button>
                                    ))}
                        ...
                        /* Search Results */
                        <div>
                            <h3 className="text-[14px] md:text-[15px] font-semibold text-zinc-800 mb-2.5 md:mb-4">Search Results</h3>
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-y-1 md:gap-y-1.5 gap-x-3 md:gap-x-4">
                                {filteredCities.length > 0 ? (
                                    ...
                                    <div className="col-span-full py-4 md:py-6 text-center text-zinc-400 text-xs md:text-sm">No cities found for "{search}"</div>
                                )}
                            </div>
                        </div>
                    )}
